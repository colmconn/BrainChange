...
function check_for_do_not_analyze {
    ...
    if [[ -f $DATA/${subject}/00_DO_NOT_ANALYSE_${subject}_20percent.txt ]] ; then
	echo "*** Subject had to much censored data"
    fi
    if [[ -f $DATA/${subject}/3dDeconvolve.err ]] ; then
	cat $DATA/${subject}/3dDeconvolve.err
    fi
...
alignment_dir=afniRsfcPreprocessed.NL
...
if [[ $# -gt 0 ]] ; then
    ...
    subjects=$( cd ../data/processed ; gfind ./ -maxdepth 1 -type d -a -name 'bc[0-9][0-9][0-9][bc]' -printf "%f\n" | sort )
fi

subjectCount=$( echo $subjects | wc -w )

timepointLimiter=baselineAndfollowup
